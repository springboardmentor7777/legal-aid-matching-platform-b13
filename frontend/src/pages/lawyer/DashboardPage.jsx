...
const LawyerDashboardPage = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ totalClients: 0, activeCases: 0, pendingRequests: 0, resolved: 0 });
  ...
  const statCards = [
    { label: 'Total Clients', value: stats.totalClients, icon: Users, color: 'bg-primary-100 text-primary-600' },
    { label: 'Active Cases', value: stats.activeCases, icon: Briefcase, color: 'bg-blue-100 text-blue-600' },
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Welcome back, {user?.name || 'Advocate'}. Here's your practice overview.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500">No recent activity yet. Your case updates will appear here.</p>
          </div>
        ) : (
