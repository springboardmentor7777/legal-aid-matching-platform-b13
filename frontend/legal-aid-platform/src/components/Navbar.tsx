...
const Navbar: React.FC<NavbarProps> = ({ title, name, role, toggleSidebar }) => {
  ...
  return (
    <header className="bg-white border-b border-blue-100 shadow-md flex items-center p-4 sticky top-0 z-50">
      ...
      <div className="ml-auto flex items-center space-x-3">
        ...
        <Dropdown items={[
          {label:<p className="flex items-center gap-[50px] cursor-pointer">Profile <span className="">{<FaUserCircle></FaUserCircle>}</span> </p>, onClick:()=>{navigate("/profile")}},
          {label:<p className="flex items-center gap-[50px] cursor-pointer">Logout <span className="">{<FaSignOutAlt></FaSignOutAlt>}</span> </p>, onClick: ()=>{localStorage.clear(); navigate("/")}}
          ]}>
          <button className="w-10 h-10 rounded-full bg-blue-200 flex items-center justify-center text-blue-900 font-bold cursor-pointer" onClick={()=>{}}>
          {name.charAt(0)}
