  ...
  return (
    <div style={s.sidebar}>
      ...
      <div ref={dropdownRef} style={s.userWrap}>
        <div style={{ ...s.userCard, borderColor: accent + "40" }} onClick={() => setDropdownOpen(!dropdownOpen)}>
          <div style={{ ...s.avatar, background: accent + "20", color: accent }}>
            {(profile?.fullName || user?.role || "U")[0].toUpperCase()}
          </div>
          <div style={s.userInfo}>
            <div style={s.userName}>{profile?.fullName || "My Account"}</div>
            <div style={{ ...s.rolePill, background: accent + "20", color: accent }}>{user?.role}</div>
          </div>
        ...
        {dropdownOpen && (
          <div style={s.dropdown}>
            <div style={{ ...s.dropHeader, background: accent + "15", borderBottom: `1px solid ${accent}30` }}>
              ...
              <div>
                <div style={s.dropName}>{profile?.fullName || "—"}</div>
                <div style={s.dropEmail}>{profile?.email || "—"}</div>
              </div>
            </div>
            <div style={s.dropItem}>
              <span style={s.dropItemIcon}>🪪</span>
              <div><div style={s.dropLbl}>Role</div><div style={s.dropVal}>{user?.role}</div></div>
            </div>
            <div style={s.dropItem}>
              <span style={s.dropItemIcon}>🆔</span>
              <div><div style={s.dropLbl}>User ID</div><div style={s.dropVal}>#{user?.userId}</div></div>
            </div>
            <div style={s.dropDivider} />
            <Link to="/profile" style={s.dropLink} onClick={() => setDropdownOpen(false)}>✏️ Edit Profile</Link>
          ...
          return (
            <Link key={item.path} to={item.path} style={{ textDecoration: "none" }}>
              <div style={{ ...s.navItem, ...(active ? { ...s.navActive, borderLeftColor: accent } : {}) }}>
                ...
                {active && <div style={{ ...s.activeDot, background: accent }} />}
              </div>
            </Link>
          );
        })}
...
const s = {
  ...
  userName: { fontSize: "12px", color: "rgba(255,255,255,0.8)", fontWeight: "600", marginBottom: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
  ...
  navLabel: { fontSize: "13px", color: "rgba(255,255,255,0.55)", fontWeight: "500" },
